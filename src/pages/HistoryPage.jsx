...
export default function HistoryPage() {
  ...
  return <>
    <input className="input mb-4" value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar cliente, placa ou telefone" />
    <div className="space-y-3">{filtered.length === 0 ? <EmptyState title="Nenhum atendimento encontrado" /> : filtered.map(item => <div key={item.id} className="card"><div className="flex items-start justify-between gap-3"><div><h3 className="font-black">{item.cliente_nome}</h3><p className="text-sm text-slate-600">{item.veiculo} · <strong>{formatPlate(item.placa)}</strong></p><p className="text-sm text-slate-500">{item.telefone}</p></div><span className={item.status_pagamento === 'pago' ? 'badge-paid' : 'badge-pending'}>{item.status_pagamento === 'pago' ? 'Pago' : 'Pendente'}</span></div><div className="mt-3 flex items-end justify-between border-t border-slate-100 pt-3"><div className="text-xs text-slate-500"><p>Atendimento: {dateTime.format(new Date(item.data_cadastro))}</p>{item.data_pagamento && <p>Pagamento: {dateTime.format(new Date(item.data_pagamento))}</p>}</div><p className="font-black">{currency.format(Number(item.valor))}</p></div></div>)}</div>
  </>
}
