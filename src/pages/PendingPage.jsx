...
export default function PendingPage() {
  ...
  return <>
    <input className="input mb-4" value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar nome, placa, telefone ou serviço" />
    ...
    <div className="space-y-3">{filtered.length === 0 ? <EmptyState title="Nenhuma pendência encontrada" /> : filtered.map(item => <div key={item.id} className="card"><div className="flex items-start justify-between gap-3"><div><h3 className="font-black text-slate-900">{item.cliente_nome}</h3><p className="mt-1 text-sm text-slate-600">{item.veiculo} · <strong>{formatPlate(item.placa)}</strong></p><p className="text-sm text-slate-500">{item.telefone}</p>{item.descricao_servico && <p className="mt-2 text-sm text-slate-700"><strong>Serviço:</strong> {item.descricao_servico}</p>}</div><p className="shrink-0 font-black text-amber-700">{currency.format(Number(item.valor))}</p></div><div className="mt-4 grid grid-cols-2 gap-2"><a className="flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-3 text-sm font-bold text-white" href={whatsappUrl(item.telefone, MSG) || '#'} target="_blank" rel="noreferrer"><WhatsAppIcon className="h-5 w-5" /> WhatsApp</a><button onClick={() => markPaid(item.id)} disabled={working === item.id} className="min-h-11 rounded-2xl bg-[#0b1f3a] px-3 text-sm font-bold text-white disabled:opacity-60">{working === item.id ? 'Salvando...' : 'Marcar pago'}</button></div></div>)}</div>
  </>
}
